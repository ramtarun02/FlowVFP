...
function PostProcessing() {
  ...
  const {
    ...
    // VFP post-processing (shared via context for sub-component navigation)
    importMode, setImportMode,
    vfpManifest, setVfpManifest,
    vfpFormData, setVfpFormData,
    vfpFileName, setVfpFileName,
    selectedWingFlowKey, setSelectedWingFlowKey,
    selectedTailFlowKey, setSelectedTailFlowKey,
  } = useSimulationData();

  const [isLoadingTail, setIsLoadingTail] = useState(false);

  // --- VFP File State (privacy-focused: parsed entirely client-side) ---
  // importMode, vfpManifest, vfpFormData, vfpFileName live in SimulationDataContext
  const [vfpFile, setVfpFile] = useState(null);
  const [vfpPolars, setVfpPolars] = useState(null);
  const [vfpParseProgress, setVfpParseProgress] = useState(0);
  const [isParsingVfp, setIsParsingVfp] = useState(false);
  // selectedWingFlowKey / selectedTailFlowKey now live in context
  // (aliased for convenience)
  const selectedWingFlowFile = selectedWingFlowKey;
  const setSelectedWingFlowFile = setSelectedWingFlowKey;
  const selectedTailFlowFile = selectedTailFlowKey;
  const setSelectedTailFlowFile = setSelectedTailFlowKey;
  const [vfpResultFiles, setVfpResultFiles] = useState({});
  const vfpInputRef = useRef(null);

  // Derived flow lists from manifest
  const wingFlowKeys = useMemo(() => {
  ...
  const handleTailFileSelect = async (tailFile) => {
    ...
    try {
      const response = await fetchAPI('/api/post/tail-downwash', {
        method: 'POST',
        body: formData
      });
  ...
  const uploadAndParseFile = async (file, fileType) => {
    try {
      ...
      if (file.file) {
        ...
        const response = await fetchAPI('/api/simulation/file-content', {
          method: 'POST',
      ...
      // Map old underscore routes → new blueprint routes
      const _parseRouteMap = { cp: '/api/post/parse-cp', forces: '/api/post/parse-forces', dat: '/api/post/parse-dat', vis: '/api/post/parse-vis', vfp: '/api/post/parse-vfp-file' };
      const _parseRoute = _parseRouteMap[fileType] || `/api/post/parse-${fileType}`;
      const parseResponse = await fetchAPI(_parseRoute, {
        method: 'POST',
        body: formData
      });
  ...
  // Update isFileSelected to support multiple .dat files and VFP files
  const isFileSelected = (file) => {
    // VFP-mode files use { name, configKey, flowKey } instead of { path }
    const matchKey = file.path || file.name;
    if (!matchKey) return false;

    const matchFn = (selected) => {
      if (!selected) return false;
      const selKey = selected.path || selected.name;
      return selKey === matchKey;
    };

    if (Array.isArray(selectedFiles.dat)) {
      if (selectedFiles.dat.some(matchFn)) return true;
    } else if (matchFn(selectedFiles.dat)) {
      return true;
    }
    return Object.values(selectedFiles).some(matchFn);
  };
  ...
  const handleImportFolder = () => {
    // If VFP mode was active, reset it first
    if (importMode === 'vfp') {
      resetVfpState();
    }
  ...
  const handleContourPlotClick = () => {
    // In VFP mode, contour plot will handle its own file selection
    if (importMode !== 'vfp' && (!parsedCpData || !selectedLevel)) {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // ══  VFP FILE IMPORT — Privacy-focused, client-side parsing  ═════════════
  // ═══════════════════════════════════════════════════════════════════════════

  /** Shared reset for VFP-specific state. */
  const resetVfpState = useCallback(() => {
    ...
    setVfpFormData(null);
    setVfpFileName(null);
    setVfpParseProgress(0);
    setIsParsingVfp(false);
    ...
    setIsLoadingForces(false);
    setIsLoadingDAT(false);
    setCoefficients({ CL: 0, CD: 0, CM: 0 });
    setDragBreakdown({ cdInduced: 0, cdViscous: 0, cdWave: 0 });
    vfpPostStorage.clear().catch(() => {});
  }, []);

  /**
   * Import .vfp file — privacy-first.
   * The entire file is stream-parsed on the client.
   * Result file data goes into IndexedDB; nothing is uploaded.
   */
  const handleVfpFileImport = useCallback(async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;

    // If folder mode was active, reset it first
    ...
    setImportMode('vfp');
    setVfpFile(file);
    setVfpFileName(file.name);
    setIsParsingVfp(true);
    setVfpParseProgress(0);

    try {
      const { formData: fd, polars: pol, manifest: man } = await streamParseVfpForPost(
        file,
        (bytesRead, totalBytes) => {
          const pct = Math.min(99, Math.round((bytesRead / totalBytes) * 100));
          setVfpParseProgress(pct);
        }
      );
      setVfpParseProgress(100);
      setVfpFormData(fd);
      setVfpManifest(man);
      ...
      if (pol) {
        ...
        setSimulationData(prev => ({
          ...
          polars: pol,
          polarsSource: 'client',
          simName: fd?.simName || file.name,
        }));
      }
    } catch (err) {
      console.error('[PostProcessing] VFP parse error:', err);
      alert('Failed to parse .vfp file: ' + (err?.message || 'Unknown error'));
      resetVfpState();
      setImportMode('none');
    } finally {
      setIsParsingVfp(false);
  ...
  }, [importMode, resetVfpState, setSimulationData]);

  /**
   * When the user selects a flow key, build the result-file listing
   * from the manifest (instant — no server call).
   */
  useEffect(() => {
    if (importMode !== 'vfp' || !vfpManifest) return;

    const configKey = selectedWingFlowFile ? 'wingConfig' : selectedTailFlowFile ? 'tailConfig' : null;
    const flowKey = selectedWingFlowFile || selectedTailFlowFile;
    if (!configKey || !flowKey || !vfpManifest[configKey]?.[flowKey]) {
      setVfpResultFiles({});
      return;
    }

    const flowEntry = vfpManifest[configKey][flowKey];
    // Convert to the same shape as folder mode: { cp: [{name}], dat: [{name}], ... }
    const grouped = {};
    for (const [ext, fileNames] of Object.entries(flowEntry.fileTypes || {})) {
      grouped[ext] = fileNames.map(n => ({ name: n, configKey, flowKey }));
    }
    setVfpResultFiles(grouped);
    setSimulationData(prev => ({
      ...(prev || {}),
      simName: `${configKey === 'tailConfig' ? 'tail' : 'wing'} / ${flowKey}`,
    }));
  }, [importMode, vfpManifest, selectedWingFlowFile, selectedTailFlowFile, setSimulationData]);

  /**
   * Handle selecting a result file inside a VFP archive.
   * Retrieves the raw file data from IndexedDB, wraps it as a Blob,
   * and sends it to the existing server parse endpoint — only the
   * selected file travels to the server.
   */
  const handleVfpResultFileSelect = useCallback(async (file, fileType) => {
    if (!file || !['dat', 'cp', 'forces'].includes(fileType)) return;

    const { configKey, flowKey, name: fileName } = file;
    ...
    try {
      // Retrieve from IndexedDB and construct a File for upload
      const blob = await getResultFileAsBlob(configKey, flowKey, fileName);
      if (!blob) {
        throw new Error(`File "${fileName}" not found in local storage`);
      }

      // For wavedrag .dat files, handle client-side
      if (fileType === 'dat' && fileName.toLowerCase().includes('wavedrg')) {
        const text = await blob.text();
        const waveDragLines = text.match(/Total wave drag for block is CDW\(tot\) =\s*([\d.]+)/g) || [];
        let waveDragSum = 0;
        waveDragLines.forEach(line => {
          const match = line.match(/CDW\(tot\) =\s*([\d.]+)/);
          if (match) waveDragSum += parseFloat(match[1]);
        });
        setDragBreakdown(prev => ({ ...prev, cdWave: waveDragSum * 0.0001 }));
        setSelectedFiles(prev => ({
          ...prev,
          dat: Array.isArray(prev.dat) ? [...prev.dat, file] : prev.dat ? [prev.dat, file] : [file]
        }));
      ...
      // Send only this file to the server for parsing
      const formData = new FormData();
      formData.append('file', blob, fileName);
      formData.append('fileName', fileName);
      formData.append('simName', vfpFormData?.simName || vfpFile?.name || 'vfp-import');

      const routeMap = { cp: '/api/post/parse-cp', forces: '/api/post/parse-forces', dat: '/api/post/parse-dat' };
      const response = await fetchAPI(routeMap[fileType], { method: 'POST', body: formData });

      if (!response.ok) {
        const errText = typeof response.text === 'function' ? await response.text() : `Status ${response.status}`;
      ...
      if (fileType === 'cp') {
        ...
        setParsedForcesData(parsedData);
        setIsLoadingForces(false);
        setSelectedFiles(prev => ({ ...prev, forces: file }));

        // Extract coefficients from forces data
        if (parsedData.levels && Object.keys(parsedData.levels).length > 0) {
          const levelKeys = Object.keys(parsedData.levels);
          const sortedLevelKeys = levelKeys.sort((a, b) => {
            const aNum = parseInt(a.match(/\d+/)?.[0] || 0);
          ...
          if (highestLevel.coefficients) {
            setCoefficients({
              CL: highestLevel.ibeCoefficients?.CL || highestLevel.coefficients.CL || 0,
              CD: highestLevel.coefficients.CD || 0,
              CM: highestLevel.coefficients.CM || 0,
            });
          }
          setDragBreakdown(prev => ({
            cdInduced: highestLevel.vortexCoefficients?.CD || 0,
            cdViscous: highestLevel.viscousDragData?.totalViscousDrag || 0,
            cdWave: prev.cdWave,
          }));
        }
      }
    } catch (err) {
      console.error(`[PostProcessing] Error parsing ${fileType} from VFP:`, err);
      alert(`Error parsing ${fileType} file: ${err.message}`);
      if (fileType === 'cp')     setIsLoadingCP(false);
      if (fileType === 'forces') setIsLoadingForces(false);
      if (fileType === 'dat')    setIsLoadingDAT(false);
    }
  }, [vfpFormData, vfpFile, setSimulationData]);
  ...
  }, [importMode, handleCloseVfp, handleRemoveFolder]);

  // --- VFP File Explorer Render ---
  const renderVfpFileExplorer = () => {
    // Guard: need either a live File or a persisted manifest from context
    if (!vfpFile && !vfpManifest) return null;

    if (isParsingVfp) {
      return (
        <div className="flex flex-col items-center justify-center h-full p-6 text-center">
          <div className="text-blue-500 mb-4">
            <svg className="w-10 h-10 mx-auto animate-spin" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
            </svg>
          </div>
          <p className="text-gray-700 font-medium mb-2">Parsing .vfp file locally…</p>
          <div className="w-full max-w-xs bg-gray-200 rounded-full h-2.5">
            <div
              className="bg-blue-600 h-2.5 rounded-full transition-all duration-300"
              style={{ width: `${vfpParseProgress}%` }}
            />
          </div>
          <p className="text-xs text-gray-500 mt-1">{vfpParseProgress}% — your data stays in the browser</p>
        </div>
      );
    }
    ...
    if (!vfpManifest) {
      return (
        <div className="flex flex-col items-center justify-center h-full p-6 text-center">
          <p className="text-gray-600">No manifest found in .vfp file</p>
        </div>
      );
    }
    ...
    return (
      <div className="h-full flex flex-col">
        ...
        <div className="p-4 border-b border-gray-200 bg-gray-50">
          <h3 className="text-lg font-semibold text-gray-800 mb-1 truncate" title={vfpFileName || vfpFile?.name || '.vfp'}>{vfpFileName || vfpFile?.name || '.vfp'}</h3>
          <p className="text-xs text-green-600 mb-3">🔒 Parsed locally — no upload</p>
        ...
        <div className="p-4 border-b border-gray-200 bg-gray-50 space-y-3">
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Wing Flow Key</label>
            <select
            ...
            >
              <option value="">Select Wing Flow Key</option>
              {wingFlowKeys.map(k => <option key={k} value={k}>{k}</option>)}
            </select>
          </div>
          {tailFlowKeys.length > 0 && (
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Tail Flow Key</label>
              <select
                className="w-full px-3 py-2 border border-blue-300 rounded-lg text-sm"
              ...
              >
                <option value="">Select Tail Flow Key</option>
                {tailFlowKeys.map(k => <option key={k} value={k}>{k}</option>)}
              </select>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {(selectedWingFlowFile || selectedTailFlowFile) && Object.keys(vfpResultFiles).length > 0 && (
            <div>
              <h4 className="font-semibold text-blue-700 mb-2">
                {selectedWingFlowFile ? 'Wing' : 'Tail'} Result Files
              </h4>
              {Object.entries(vfpResultFiles).map(([fileType, fileList]) => {
                  ...
                  <div key={fileType} className="mb-2">
                    ...
                    <div className="space-y-1">
                      {fileList.map((file, index) => {
                        ...
                        const isParseable = ['dat', 'cp', 'forces'].includes(fileType);
                        return (
                          <div
                            key={`${file.name}-${index}`}
                            ...
                            onClick={() => {
                              if (!isLoading && isParseable) {
                                handleVfpResultFileSelect(file, fileType);
                              }
                            }}
                            title={file.name}
                          >
      ...
      return (
        <div className="flex flex-col items-center justify-center h-full p-6 text-center">
          ...
          <div className="flex flex-col space-y-2 w-full max-w-xs">
            ...
            <button
              onClick={() => vfpInputRef.current?.click()}
              className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
            >
              Import .vfp File
            </button>
          </div>
    ...
    <div className="h-screen w-screen flex flex-col bg-blue-50 font-sans">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-blue-200 shadow-sm">
        ...
        <div className="flex items-center space-x-3">
          {/* Hidden file input for .vfp import (triggered from side panel) */}
          <input
            ref={vfpInputRef}
            type="file"
            accept=".vfp,.json"
            style={{ display: 'none' }}
            onChange={handleVfpFileImport}
          />
          <button
          ...
          <button
            ...
            title={importMode === 'vfp' ? 'Close VFP File' : 'Close Folder'}
          >
            {importMode === 'vfp' ? 'Close VFP' : 'Close Folder'}
          </button>
          <button
