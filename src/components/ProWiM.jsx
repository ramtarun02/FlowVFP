...
// Rounding helpers
const roundRatio = (val) => { const n = parseFloat(val); return Number.isFinite(n) ? Math.round(n * 10000) / 10000 : 0; };  // 4 dp  – ratios
const roundAngle = (val) => { const n = parseFloat(val); return Number.isFinite(n) ? Math.round(n * 100) / 100 : 0; };        // 2 dp  – angles
const roundOther = (val) => { const n = parseFloat(val); return Number.isFinite(n) ? Math.round(n * 1000) / 1000 : 0; };      // 3 dp  – everything else
...
function PropellerWingForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetchAPI("/api/prowim/compute", {
        method: "POST",
