...
# # Suppress all logs except errors from noisy libraries
...
# def run_vfp():
#     """Handle VFP simulation start request compatible with both runSolver and Solver pages"""
#     try:
#         # Get form data
...
#         continuation = request.form.get("continuation") in ["true", "True", "1", "on"]
#         excrescence = request.form.get("excrescence") in ["true", "True", "1", "on"]
#         autoRunner = request.form.get("autoRunner") in ["true", "True", "1", "on"]
#         autoStepSize = request.form.get("autoStepSize")
#         autoEndAoA = request.form.get("autoEndAoA")
#         autoEndMach = request.form.get("autoEndMach")
#         autoMode = request.form.get("autoMode")
#         dumpName = request.form.get("dumpName")
#         bodyFiles = request.form.get("bodyFiles")
...
#                 files_received[file_key] = file.filename

#         # Prepare user_inputs for both pages
#         user_inputs = {
#             "simName": simName,
#             "mach": mach,
#             "aoa": aoa,
...
#             "autoStepSize": autoStepSize,
#             "autoEndAoA": autoEndAoA,
#             "autoEndMach": autoEndMach,
#             "autoMode": autoMode,
#             "dumpName": dumpName,
#             "bodyFiles": bodyFiles,
#         }

#         # Optionally add solver flags if present
#         solverFlags = []
#         if autoRunner:
#             if autoMode == "aoa" and autoEndAoA:
#                 solverFlags.append(f"--sweep-aoa {autoStepSize} {autoEndAoA}")
#             elif autoMode == "mach" and autoEndMach:
#                 solverFlags.append(f"--sweep-mach {autoStepSize} {autoEndMach}")
#         if excrescence:
#             solverFlags.append("--excrescence")
#         if continuation and dumpName:
#             solverFlags.append(f"--restart {dumpName}")
#         user_inputs["solverFlags"] = solverFlags
...
#     sid = request.sid
#     try:
#         # 1. Validate the file name provided
...
#         print(data['wing_GEO'], data['wing_MAP'], data['wing_DAT'])

#         wing_geo = data.get('wing_GEO')
#         wing_map = data.get('wing_MAP')
#         wing_dat = data.get('wing_DAT')
#         tail_geo = data.get('tail_GEO')
#         tail_map = data.get('tail_MAP')
#         tail_dat = data.get('tail_DAT')
#         tail_0 = data.get('tail_0')        
#         dump_file = data.get('dumpName')
        
#         # print(wing_geo, wing_map, wing_dat, tail_geo, tail_map, tail_dat, tail_0, dump_file)

#         # Validate wing files
#         missing_files = []
#         for fname in [wing_geo, wing_map, wing_dat]:
#             if fname and not (sim_folder / fname).exists():
#                 missing_files.append(fname)
#         if missing_files:
#             emit('error', f"Missing required wing files: {', '.join(missing_files)}")
#             return

#         # Validate tail files for multi-stage
#         multi_stage_ready = all([
#             tail_geo and (sim_folder / tail_geo).exists(),
#             tail_map and (sim_folder / tail_map).exists(),
#             tail_dat and (sim_folder / tail_dat).exists(),
#             tail_0 and (sim_folder / tail_0).exists()
#         ])

#         # 3. Remove file extensions for batch file arguments
#         def strip_ext(filename):
#             return os.path.splitext(filename)[0] if filename else ''
#         map_file = strip_ext(wing_map)
#         geo_file = strip_ext(wing_geo)
#         dat_file = strip_ext(wing_dat)
#         dump_file_base = strip_ext(dump_file) if dump_file else ''

#         # 4. Copy necessary files to simulation folder (only utils folder)
#         utils_folder = project_root / "modules" / "utils"
#         if utils_folder.exists():
#             copy_files_to_folder(str(utils_folder), str(sim_folder))

#         # 5. Decision logic based on user inputs
#         continuation = str(data.get("continuation", "false")).lower() == "true"
#         auto_runner = str(data.get("autoRunner", "false")).lower() == "true"
#         excrescence = str(data.get("excrescence", "false")).lower() == "true"
...
#         # 6. Auto Runner Mode
#         if auto_runner and not continuation and not excrescence and not multi_stage_ready:
#             emit('message', "Running in Auto Runner mode...")
#             dalpha = str(data.get("autoStepSize", "1"))
#             alphaN = str(data.get("autoEndAoA", "1"))
#             stream_process_threaded([
#                 sys.executable, "VFP_Full_Process.py",
#                 wing_dat, dalpha, alphaN, map_file, geo_file
#             ], str(sim_folder))
#             return

#         # 7. Continuation Mode
#         if continuation and not auto_runner and not excrescence and not multi_stage_ready:
#             emit('message', "Running in Continuation mode...")
#             if not dump_file_base:
#                 emit('error', "Dump file name is required for continuation run")
#                 return
#             dump_file_path = sim_folder / f"{dump_file_base}.fort52"
#             if not dump_file_path.exists():
#                 emit('error', f"Dump file '{dump_file_base}.fort52' not found in simulation folder")
#                 return
#             emit('message', f"Found dump file: {dump_file_base}.fort52")
#             bat_args = [map_file, geo_file, dat_file, "n", "y", dump_file_base]
...
#         if multi_stage_ready and not continuation and not auto_runner and not excrescence:
#             emit('message', "Multi Stage simulation detected. Running multi-stage computation...")

#             # Import here to avoid circular import at module level
#             import sys
#             sys.path.insert(0, str(project_root / "modules"))
#             from modules.airVFP import multi_stage_tail_downwash

#             # Prepare full paths for all files
#             wing_geo_path = str(sim_folder / wing_geo)
#             wing_map_path = str(sim_folder / wing_map)
#             wing_dat_path = str(sim_folder / wing_dat)
#             tail_geo_path = str(sim_folder / tail_geo)
#             tail_map_path = str(sim_folder / tail_map)
#             tail_dat_path = str(sim_folder / tail_dat)
#             tail_0_path   = str(sim_folder / tail_0)

#             def emit_message(msg, sid):
#                 socketio.emit('message', msg, room=sid)

#             def run_multi_stage(sid):
#                 try:
#                     results = multi_stage_tail_downwash(
#                         str(sim_folder),
#                         wing_geo_path, wing_map_path, wing_dat_path,
#                         tail_geo_path, tail_map_path, tail_dat_path, tail_0_path,
#                         emit_message=lambda msg: emit_message(msg, sid)
#                     )
#                     emit_message("[Multi-Stage] Computation finished.", sid)
#                     emit_message(f"[Multi-Stage] Results: {results}", sid)
#                     socketio.emit('done', {'success': True, 'results': results}, room=sid)
#                 except Exception as e:
#                     emit_message(f"[Multi-Stage] Error: {str(e)}", sid)
#                     logger.error(f"Multi-stage error: {e}")

#             thread = threading.Thread(target=run_multi_stage, args=(sid,))
#             thread.daemon = True
#             thread.start()
#             return
...
#         # 9. Standard Mode (wing alone case)
#         if not continuation and not auto_runner and not excrescence and not multi_stage_ready:
#             emit('message', "Running in Standard mode - wing alone case...")
#             bat_args = [map_file, geo_file, dat_file, "n", "n", ""]
#             emit('message', f"Batch arguments: {' '.join(bat_args)}")
#             stream_bat_process_threaded(str(bat_file_path), str(sim_folder), bat_args)
#             return

#         # 10. Excrescence Mode (not implemented)
#         if excrescence and not continuation and not auto_runner and not multi_stage_ready:
#             emit('error', "Excrescence mode is not implemented yet.")
#             return

#         # 11. Handle invalid combinations
#         if continuation and auto_runner:
#             emit('error', "Cannot run both Continuation and Auto Runner simultaneously")
#         elif continuation and excrescence:
#             emit('error', "Cannot run both Continuation and Excrescence simultaneously")
#         elif auto_runner and excrescence:
